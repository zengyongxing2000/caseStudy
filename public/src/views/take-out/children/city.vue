<template>
  <div class="page-city">
    ...
    <div v-for="(value, key, index) in sortCity" :key="key" class="letter_classify_li">
      <h4 class="city_title">{{key}}
        <span v-if="index == 0">（按字母排序）</span>
      </h4>
      <!-- <router-link v-for="item in value" :to="'/city/' + item.id" :key="item.id" class="ellipsis"> -->
      <span class="citywrap color99" v-for="item in value" :key="item.id"> <span class="city">{{item.name}} </span>
      </span>
      <!-- </router-link> -->
    </div>
